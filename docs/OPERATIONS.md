...
| Did Trivy die or time out? | `subprocess_exits_total{reason}`, `subprocess_exit_code_total{code}` | `success` | Failure category spike |
...
Trivy writes under its own `$TMPDIR/trivy-<random>` (DB downloads, post-analyzer copies, files over 100 MiB, materialised layers with `--max-image-size`). It cleans up on a normal exit or SIGTERM, never after SIGKILL, and the suffix is random rather than a pid, so nothing else can tell a running scan's directory from an abandoned one. The adapter therefore points each child's `TMPDIR` at a directory it made under its own root, `$TMPDIR/harbor-scanner-trivy-<random>/`, and removes it once the child is gone, however it died. The root's name is random rather than the pid, because `/tmp` outlives a container restart while the adapter returns as pid 1. The owner touches its root every two minutes, so only a root nothing has refreshed for ten minutes is considered ownerless; those are removed at startup and every ten minutes (`temp_dirs_reaped_total`). A clean shutdown removes the root outright. `storage_available_bytes{area="tmp"}` reports the temp filesystem and `cache_size_bytes{kind="tmp_trivy"}` what this adapter's children hold. Point `TMPDIR` at the cache volume if the container's writable layer is small.
