...
- `queue_collection_success` = 0 with `queue_collection_errors_total{query="group"}` rising: the consumer group is gone. The stream and its deliveries may well survive, so this is lost group state rather than lost data: check the job Redis for a restart without persistence, and for a manual `XGROUP DESTROY`. The worker recreates the group and counts `queue_group_recreated_total`; on older builds restart the StatefulSet.
