...
| Did Trivy die or time out? | `subprocess_exits_total{reason}`, `subprocess_exit_code_total{code}`; the recurring `version` probe reports through the same series, so select `command="image"` or `command="sbom"` | `reason="success"`, `code="0"` | Failure category spike |
...
- `queue_quarantined_jobs` > 0: a delivery could not be decoded, or its job key was gone while the delivery was not (an eviction policy that touches operational keys, or a manual delete). Inspect `<namespace>:stream:v1:scan_artifact:quarantine` with `redis-cli`: each entry carries a `reason` next to the original `fields` (which contain registry credentials, treat as secrets). Fix the producer, delete the entry.
