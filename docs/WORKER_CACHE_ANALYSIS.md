...
Historical analysis of adapter commit `9891e7a`, which pins Trivy `0.74.0`, prepared on 2026-09-09. Findings come from source inspection; no production workload or Redis capacity benchmark was run.

**Implementation update:** the adapter now enforces one worker per pod, forwards its cache configuration to Trivy, and uses durable Redis Streams. The configuration gaps and Pub/Sub behavior below describe the baseline commit, not current behavior. Use the [scaling guide](SCALING.md) for supported deployment settings and validation results. In particular, native `TRIVY_*` cache overrides below have been superseded by `SCANNER_TRIVY_CACHE_*`.

The adapter already starts multiple workers when configured. Its default filesystem scan cache prevents those workers from reliably running image scans against the same cache directory. Redis removes that shared-file lock and allows workers across pods to reuse analysis. Redis memory will not equal the size of `fanal.db`: retention, database overhead, and replication determine the difference.
...
There are two scaling dimensions:
...
These are execution slots, not promised throughput. Harbor must submit enough outstanding work, and CPU, memory, temporary storage, registry bandwidth, and cache latency must support it. Each Trivy process can also parallelize its own work. Increasing adapter workers multiplies that resource demand.

Across replicas, Pub/Sub broadcasts each job to every subscribed adapter; the Redis job lock arbitrates execution. Replicas must use the same job/store configuration and namespaces. This is not a Redis consumer-group queue. [Enqueuer implementation](https://github.com/container-registry/harbor-scanner-trivy/blob/9891e7a/pkg/queue/enqueuer.go)

## Why fanal.db obstructs concurrency

Trivy opens `fanal.db` as a writable BoltDB database. That takes an exclusive process-level file lock, retained while the database is open. It is not just a brief lock around inserting one package record. Another Trivy process using the same file waits for the lock and, in v0.74.0, fails after approximately five seconds if it cannot acquire it. See the [filesystem cache implementation](https://github.com/aquasecurity/trivy/blob/v0.74.0/pkg/cache/fs.go).
...
Use a separate Redis instance for this cache where practical. The adapter's existing Redis contains job locks, state, and reports that must remain available until consumed. A cache eviction policy on that same instance can remove those records. A different Redis database number separates keys, but does not create an independent memory budget or eviction policy. Even a volatile policy can select adapter records because those records also have TTLs. This recommendation follows from the adapter's [store implementation](https://github.com/container-registry/harbor-scanner-trivy/blob/9891e7a/pkg/persistence/redis/store.go) and Redis's server-wide eviction configuration.

Redis adds network latency and availability dependence. Expiration or eviction between a cache check and later layer application may also produce a missing-layer error; test under memory pressure rather than assuming every eviction is merely a slower scan. Cache write errors can fail scans. Keep enough capacity for active scans and useful reuse.

## Current configuration gap and a usable path
...
There is an existing path without modifying the binary: the wrapper inherits its environment, and Trivy binds its own `TRIVY_CACHE_BACKEND` and `TRIVY_CACHE_TTL` variables. The following is a configuration example for evaluation, not a deployed or load-tested change. Replace the example hostname with a provisioned cache service:
...
Illustrative sizing only: if the workload writes 10,000 distinct layer entries per day averaging 100 KiB of JSON, and entries remain for seven days without eviction, their payload is about 6.7 GiB. Image-level records and all memory overhead come on top. At that same hypothetical rate, 90 days of retained history would hold about 85.8 GiB of layer payload. The reduction comes from retention, not Redis compression. Actual repeated-key writes, TTL expiry, workload variation, and eviction change these figures.
...
Redis caching addresses the analysis-cache lock. Several independent behaviors still need validation or changes:
...
Treat server mode as an adapter integration project: verify image scans, SBOM generation, SBOM accessory scans, credentials, enabled scanners, and findings parity before switching. Prefer the simpler replica architecture until measurements justify the additional tier.
