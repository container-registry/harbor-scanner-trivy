...
| | Before | After | Change |
|---|---|---|---|
| "Scan All" wall clock | 1h54m | 1h19m | **−30%** (34 min) |
| Peak Redis memory | 1,020 MB | 274 MB | **−73%** |

**Vulnerability scans can be served from an existing SBOM accessory** ([#38], opt-in). If Harbor already attached an
SBOM generated by this adapter, the scan reads that ~1 MB SBOM through the OCI referrers API instead of pulling and
analyzing every layer. Across 14 images:

| Image | Cold scan | SBOM scan | Speedup |
|---|---|---|---|
| `node:18.20.5` | 7.9s | 0.6s | 13.2x |
| `node:22.14.0-slim` | 2.7s | 0.3s | 9.0x |
| `tensorflow/tensorflow:2.18.0` | 12.4s | 0.9s | 13.8x |
| `jenkins/jenkins:2.479.3-lts-jdk17` | 8.7s | 0.3s | 29.0x |
| `homeassistant/home-assistant:2025.1.0` | 17.4s | 0.5s | 34.8x |

Range across all 14: **6.0x to 34.8x**. Findings are identical for 13 of 14 images; the one divergence
(`apache/airflow`) is a strict superset on the SBOM side, so no finding is lost. Any missing accessory, lookup
failure or failed SBOM scan falls back to a full image scan. Reproduce with `test/benchmark/sbom-accessory-bench.sh`.
